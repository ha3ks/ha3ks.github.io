## 📊 Grafana-Style Blog Dashboard

A personal blog homepage built using a Grafana-inspired dashboard UI.

This project uses GitHub Pages + Jekyll to power a static blog and displays a dynamic Grafana-style dashboard as the homepage.
The dashboard automatically updates to show:

* 📄 Latest blog posts

* 🧁 Posts by category (donut chart)

* 📅 Calendar with post dates

* 🟩 GitHub commit activity

* ℹ️ “About Me” panel with image

* 🔄 Auto-refreshing data panels

Everything is generated on the client side using JavaScript, with Jekyll providing JSON feeds (posts.json and tags.json) directly from your _posts folder.

## 🚀 Features

 ✔ Dynamic Dashboard Panels

Panels update automatically every 60 seconds. No backend required.

 ✔ Live Blog Integration

When you add a new blog post in _posts/, the dashboard automatically updates:

* Total post count
* Tag/category count
* Donut chart
* Latest 5 posts list
* Calendar highlighting post dates

 ✔ GitHub Activity

Shows the total commits made in your repository over the last 7 days.

 ✔ About Panel

Circular profile picture + text panel, centered and styled to match the dashboard theme.

 ✔ 100% GitHub Pages Compatible

No plugins needed.
Just HTML, CSS, JavaScript, and Jekyll’s built-in Liquid templating.

## 📁 Project Structure

```bash
/
├── index.html          # Grafana-style homepage dashboard
├── style.css           # Dashboard styles
├── script.js           # JS logic for charts, refresh, GitHub API, etc.
├── posts.json          # Auto-generated by Jekyll (blog posts)
├── tags.json           # Auto-generated by Jekyll (tag counts)
├── _posts/             # Your actual blog posts (Markdown files)
└── assets/
    └── profile.jpg     # About panel profile picture
```

## 📝 How Blog Posts Work

All blog posts live in the ```_posts/``` folder using standard Jekyll formatting:
```yaml
_posts/
  2025-01-01-my-post.md
  2025-01-02-another-post.md
```

Each post contains YAML front matter:
```yaml
---
title: "My Example Post"
date: 2025-01-01
tags: [developer, personal]
---
```

Jekyll automatically parses posts and generates:

```posts.json``` — metadata for all posts

```tags.json``` — tag counts

Your dashboard fetches these and updates panels automatically ✨

## 🧩 How the Dashboard Works

## 1. Stat Panels

Managed in ```script.js```.

They read data from:

* ```posts.json```

* GitHub API

* tags extracted from posts

These update every 60 seconds via:
```js
setInterval(updateDashboard, REFRESH_INTERVAL);
```
---
## 🎨 How to Modify Dashboard Panels

You can modify or add new panels in three steps:

### 🟦 1. Edit or Add HTML Panels (index.html)

Each panel is a .panel element:
```html
<div class="panel text-panel">
  <div class="panel-title">Latest Blog Posts</div>
  <ul id="postsList"></ul>
</div>
```

Create new panels by duplicating this block.

Give each panel a unique ID or class so JavaScript can target it.

## 🟩 2. Add or Modify JavaScript Logic (script.js)

Each data source is handled in ```updateDashboard()```.

Examples:

* List latest posts
* Limit posts to 5
* Update stats
* Build charts
* Mark dates on the calendar
* Fetch GitHub commits

To create a new panel, write JavaScript that outputs into the HTML container:
```js
document.getElementById("myPanel").textContent = "Hello!";
```
Or create new Chart.js panels, tables, etc.

## 🟥 3. Style It (style.css)

Panels use reusable styles:
```css
.panel { background:#242424; padding:20px; border-radius:6px; }
.panel-title { text-transform: uppercase; font-size: 13px; }
```

You can modify the look or create custom classes:
```css
.my-custom-panel { background:#303030; }
```

## 🧭 Adding New Panels (Examples)

✔ Quote Panel

```HTML:

<div class="panel" id="quotePanel">
  <div class="panel-title">Quote of the Day</div>
  <p id="quoteText"></p>
</div>
```

```JS:

document.getElementById("quoteText").textContent = "Stay curious.";
```

### ✔ Posts-per-Month Chart

Use Chart.js + post dates.

### ✔ Tag Cloud Panel

Generate a visual tag cloud sized by tag frequency.

## 🔧 Configuration
Update GitHub API URL in script.js:
```
fetch("https://api.github.com/repos/<USERNAME>/<REPO>/commits")
```

Replace with your GitHub username and repo name.

## 📦 Deployment on GitHub Pages

1, Push the project to GitHub

1, Go to Settings → Pages

3, Set:
```
Source: main branch
Folder: root (/)
```

4, Save

5, Your site appears at:

```
https://<USERNAME>.github.io/<REPO>/
```

## 🎉 Final Notes

This project combines:

* Jekyll (for blog content)
* JavaScript (for dashboard logic)
* Chart.js (for charts)
* GitHub Pages (for hosting)